...
## Вывод по работе. 
- Я смог создать сервер на языке С++  отвечающий  за взаимодействие с Алисой, а так же за обработку и дальнейшую пересылку данных подчинённым сервисам  и отправки  html страницы с последующим созданием запроса в формате json
- Так же создал клиентское приложения на языке Python, получающее и обрабатывающее данные из сервера в excel-таблицу.